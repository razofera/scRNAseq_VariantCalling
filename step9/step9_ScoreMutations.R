...
Reads <- read_tsv(args[2], col_types = "cdccccdcc", col_names = FALSE)
...
FilteredMutations <- score.mutations(Mutations, Reads, Metadata)

# Write scored mutations to drive:
write.csv(FilteredMutations, file = 'ScoredMutations.csv', row.names = F)
